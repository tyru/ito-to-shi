...
(function ($window, $document) {
  ...
  var init = function init() {
    ...
    var svgDS = getSvgDS();
    $svg = _d2.default.select("body").select("svg").on('touchstart keydown mousedown', function () {
      return screenDispatcher.touchStart();
    }).on('touchend keyup mouseup', function () {
      return screenDispatcher.touchEnd();
  ...
  var makeSelectModeScreen = function makeSelectModeScreen() {
    ...
    ctx.selectModeButtonRectDS = [{ x: 40, y: 30, fill: 'green', width: 110, height: 70 }, { x: 180, y: 30, fill: 'blue', width: 110, height: 70 }, { x: 40, y: 130, fill: 'red', width: 110, height: 70 }, { x: 180, y: 130, fill: 'purple', width: 110, height: 70 }];
    ctx.selectModeButtonTextDS = [{ x: 60, y: 70, fontSize: '24px', text: 'EASY', fill: 'white' }, { x: 190, y: 70, fontSize: '24px', text: 'NORMAL', fill: 'white' }, { x: 60, y: 170, fontSize: '24px', text: 'HARD', fill: 'white' }, { x: 190, y: 170, fontSize: '24px', text: 'LUNATIC', fill: 'white' }];
  };
  ...
  var drawSelectModeScreen = function drawSelectModeScreen($selectModeScreen) {
    ...
    $selectModeScreen.selectAll('rect.selectModeButtonRectDS').data(ctx.selectModeButtonRectDS).enter().append('rect.selectModeButtonRectDS').attr('data-mode', function (_, i) {
  ...
  var moveNeedles = function moveNeedles() {
    ...
    if (willMove >= 0) {
      ...
      var level = getCurrentScore() + rightsideNeedleNum >= nextLvScore ? ctx.level + 1 : ctx.level;
      ...
      if (level > ctx.level) {
        ...
        assert(nextNeedleNum >= ctx.needleGroupDS.length, 'Lv.UP must not cause getMmByLevel() to be smaller number');
        if (nextNeedleNum > ctx.needleGroupDS.length) {
          // Re-calculate the necessary number of needles.
          var necessaryNeedleNum = nextNeedleNum - ctx.needleGroupDS.length;
  ...
  var drawNeedles = function drawNeedles($needles) {
    ...
    $needles.each(function (d) {
      ...
      _d2.default.select(this).transition().duration(shouldAnimate(d) ? THIRTY_FPS : 0).attr('transform', 'translate(' + d.x + ',' + d.y + ')');
    });
  ...
  var drawThread = function drawThread($thread) {
    ...
    // Update
    $thread.transition().duration(shouldAnimate() ? THIRTY_FPS : 0).attr('transform', function (d) {
